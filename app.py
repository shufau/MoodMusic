...
with st.expander("請點擊此處為我們評分"):
    ...
    with st.form("my_feedback_form", clear_on_submit=True):
        ...
        user_comment = st.text_area("其他建議：",placeholder="例如：私心推薦不好聽、搜尋結果過濾可以寬鬆一點...",help="請放心填寫，您的建議將會匿名傳送給我們，作為未來優化的重要參考。", key="user_msg")
...
st.markdown("""
    ...
    @keyframes floatAnimation {
        ...
        25% { transform: translate(100px, 150px) rotate(5deg); }
        50% { transform: translate(250px, 50px) rotate(-5deg); }
        75% { transform: translate(150px, 250px) rotate(3deg); }
    ...
    .floating-text-box {
        ...
        z-index: 1000; /* 確保在一般元件之上 */
        pointer-events: none; /* 讓滑鼠可以穿過它，不影響點擊下方按鈕 */
